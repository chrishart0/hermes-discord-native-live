...
from discord_native_live.live import LiveConnection, append_chunks
...
async def test_delegation_uses_transcript_not_missing_event_text():
    calls = []
    live = LiveConnection(lambda *args: calls.append(args), lambda _: None)
    live.ws = Socket()
...
async def test_results_remain_bound_when_b_finishes_first():
    ...
    live.ws = Socket()
    await live.speak("b", "B done")
    await live.speak("a", "A done")
...
async def test_only_native_live_protocol_events_and_raw_pcm():
    pcm = b"\x01\0" * 480
    received = []
    live = LiveConnection(lambda *_: None, received.append)
    live.ws = Socket()
...
async def test_two_delegations_do_not_block_receiver_on_work():
    ...
    async def work(key):
        ...
        if key == "a":
            await barrier.wait()
    live = LiveConnection(lambda key, *_: tasks.append(asyncio.create_task(work(key))), lambda _: None)
    live.ws = Socket()
    for key in ["a", "b"]:
