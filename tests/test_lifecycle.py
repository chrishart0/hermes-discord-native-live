...
async def test_discord_failure_does_not_skip_billed_provider_close(host):
    ...
    session.voice_client = object()
    host.adapter._voice_clients = {session.guild_id: session.voice_client}
    called = []
...
async def test_cleanup_restores_only_owned_receiver_hooks(host):
    from discord_native_live.audio import ReceiverTap
    plugin = types.SimpleNamespace(ctx=Context(), sessions={})
    session = DiscordVoiceSession(plugin, host.native, Event('join'), host.voice.channel, object())
    original_map = lambda *_: None
    original_buffers = {}
    receiver = types.SimpleNamespace(_lock=threading.Lock(), _buffers=original_buffers,
                                    map_ssrc=original_map)
    session.tap = ReceiverTap(receiver, session.capture)
    host.adapter._voice_clients = {}
    await session.close()
    assert receiver._buffers is original_buffers
