...
from discord_native_live.native import current_work, TaskOwner
...
async def test_b_finishes_while_a_is_held_in_native_admission(host):
    ...
    assert a_key != b_key and a.thread_id != b.thread_id
    assert a.turn_status == "running" and not a_task.done() and not a_task.cancelled()
    assert host.gateway.finished == ["B"]
    assert [(j.delegation_id, j.text) for j in host.notices] == [("delegation-B", "answer:B")]
    assert host.adapter.queued == []
    gate.set()
    await finished(host)
    assert [(j.delegation_id, j.text) for j in host.notices] == [("delegation-B", "answer:B"), ("delegation-A", "answer:A")]
...
async def test_model_is_pinned_without_credentials_and_provenance_survives(host):
    ...
    assert host.adapter.text_results == [(job.thread_id, "answer:test")]


async def test_duplicate_completion_not_announced_again(host):
    job = await host.native.submit(host.voice, "a", "test", "")
    await finished(host)
...
async def test_foreign_user_profile_and_channel_cannot_control(host):
    ...
    for changed in [dict(user_id="99"), dict(profile="beta"), dict(chat_id="999")]:
        foreign = TaskOwner.from_source(dataclasses.replace(host.voice.source, **changed))
        with pytest.raises(PermissionError):
            await host.native.control(foreign, "cancel", job.thread_id)
...
async def test_explicit_control_uses_native_command_on_exact_thread(host):
    gate = host.gateway.barriers["A"] = asyncio.Event()
    job = await host.native.submit(host.voice, "a", "A", "")
    await host.native.control(job.owner, "steer", job.thread_id, "Focus on tests")
    await host.native.control(job.owner, "cancel", job.thread_id)
    assert host.adapter.controls == [("/steer Focus on tests", job.thread_id), ("/stop", job.thread_id)]
...
async def test_no_native_admission_is_not_success(host):
    ...
    assert next(iter(host.native.tasks.values())).turn_status == "dispatch failed"
...
async def test_runner_context_is_task_scoped(host):
    ...
    async def see_context(message, context_prompt, history, source, session_id, **kwargs):
        seen.append(current_work.get())
        return {"final_response": "ok"}
    host.native.unload()
    host.gateway._run_agent = see_context
    from discord_native_live.native import HermesTaskBridge
    host.native = HermesTaskBridge(host.gateway, host.adapter)
    host.native.attach(lambda _: False)
    host.voice.bridge = host.native
    a = await host.native.submit(host.voice, "a", "A", "")
    b = await host.native.submit(host.voice, "b", "B", "")
    await finished(host)
    assert seen == [(host.native, a.owner), (host.native, b.owner)]
...
async def test_second_adapter_observer_can_wrap_same_gateway(host):
    ...
    from discord_native_live.native import HermesTaskBridge
    other = HermesTaskBridge(host.gateway, Adapter(host.gateway))
    other.attach(lambda _: False)
    job = await host.native.submit(host.voice, 'a', 'A', '')
    await finished(host)
    assert host.notices[0].text == 'answer:A'
    other.unload()
